...
function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/NotesPage"), 2000);
    } catch (error) {
      if (error.response) {
        showAlert("error", error.response.data.msg || "Login gagal");
      }
    }
  };
      ...
      <div className="w-full max-w-7xl h-auto md:h-[650px] bg-[#2B2B2B] rounded-3xl md:rounded-[53px] flex flex-col md:flex-row overflow-hidden shadow-lg">
        ...
        <div className="relative w-full text-white flex flex-col justify-center items-center p-8 md:p-12">
          <form
            ...
            className="w-full max-w-sm space-y-4 md:-translate-y-7"
          >
            ...
            <p className="text-center text-sm text-gray-400 pt-4">
              Tidak punya akun?{" "}
              <Link to="/RegisterPage" className="text-[#3471FF] font-medium hover:underline">
                Daftar akun baru
              </Link>
            </p>
          </form>
